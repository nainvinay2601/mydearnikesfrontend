...
const LatestDrop = () => {
  ...
  useEffect(() => {
    const fetchLatestProducts = async () => {
      try {
        setLoading(true);
        const latestProducts = await getLatestProducts(8); // Only fetch 6 products
  ...
  if (loading) {
    return (
      <>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 bg-gray-100 border-t-[0.25px] border-[#aeadad] auto-rows-fr">
          {[...Array(8)].map((_, index) => (
            <div
              key={index}

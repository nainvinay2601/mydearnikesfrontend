...
const Header = () => {
  ...
  // 🔥 NEW: Function to close menu - pass this to child components
      ...
      <header className="fixed top-0 left-0 right-0 z-50 bg-white borde-b-[0.5px] border-[#aeadad]">
        ...
        {active && (
          <div className="absolute top-full left-0 right-0  ">
            ...
            {/* Section bodies - 🔥 PASS closeMenu function to child components */}
            {active === "tees" && <TeesComponent onClose={closeMenu} />}
            {active === "newin" && <NewInComponent />}
            {active === "bottoms" && <BottomComponent onClose={closeMenu} />}
            {/* {active === "all" && <AccessoriesComponent onClose={closeMenu} />} */}
            {active === "search" && <SearchComponent />}

...
//             className="border-b-[0.25px] border-r-[0.25px] border-gray-400 flex flex-col group cursor-pointer hover:bg-gray-50 transition-colors duration-200"
//           >
//             <div className="flex-1 bg-gray-100 flex justify-center items-center  group-hover:bg-gray-200 transition-colors duration-200 ">
//               {product.featuredImage ? (
//                 <Image
//                   src={product.featuredImage.url}
//                   alt={product.featuredImage.altText || product.title}
...
//     setTimeout(() => {
//       setVisibleItems((prev) => Math.min(prev + 8, filteredProducts.length));
//       setIsLoadingMore(false);
//     }, 500);
//   }, [isLoadingMore, visibleItems, filteredProducts.length]);
...
//     const loadTrigger = document.getElementById('product-filter-load-trigger');
//     if (loadTrigger) {
//       observer.observe(loadTrigger);
//     }

//     return () => observer.disconnect();
//   }, [loadMore, visibleItems, isLoadingMore, filteredProducts.length]);
...
//   // Slice filtered products for infinite scroll
...
//       {/* Heading Section */}
//       <div className="headingClass py-4 px-[8px] tracking-tight font-semibold text-md">
...
//       {/* Product Grid */}
//       <div className="grid grid-cols-2 md:grid-cols-3 bg-gray-100 border-[0.25px] border-b-[0.125px] border-[#aeadad] auto-rows-fr">
//         {visibleProducts.map((product: SimpleProduct) => (
//           <Link
//             key={product.id}
//             href={`/product/${product.handle}`}
//             className="border-b-[0.25px] border-r-[0.25px] border-gray-400 flex flex-col group cursor-pointer hover:bg-gray-50 transition-colors duration-200"
...
//       {hasMore && (
//         <div 
//           id="product-filter-load-trigger" 
...
//       {/* 🔥 End message when all filtered products are loaded */}
...
//       {showFilterModal && (
//         <div className="fixed bottom-0 left-1/2 transform -translate-x-1/2 w-full z-50">
//           <div className="bg-white rounded-3xl px-[12px] py-4 overflow-y-auto border-[0.25px] border-[#aeadad] mx-[8px] mb-2 max-h-[80vh]">
//             <div className="flex items-center justify-center mb-2">
//               <h3 className="text-2xl tracking-tight uppercase">{categoryTitle}</h3>
//             </div>
//             <button
...
const ProductFilter: React.FC<ProductFilterProps> = ({ products, categoryTitle }) => {
  ...
  */
  ...
  return (
   ...
   {/* Product Grid */}
<div className="grid grid-cols-2 md:grid-cols-3 bg-gray-100 border-[0.25px] border-b-[0.125px] border-[#aeadad]">
  {visibleProducts.map((product: SimpleProduct) => (
    <Link
      key={product.id}
      href={`/product/${product.handle}`}
      className="border-b-[0.25px] border-r-[0.25px] border-gray-400 flex flex-col group cursor-pointer hover:bg-gray-50 transition-colors duration-200"
    >
      {/* Square Image Container - No Empty Space */}
      ...
      <div className="px-1 bg-white py-1 flex border-t-[0.5px] border-gray-200 justify-between items-center text-sm group-hover:bg-gray-50 transition-colors duration-200 ">
        <h3 className="font-inter text-xs font-normal group-hover:text-gray-700 transition-colors duration-200 truncate uppercase flex-1">
          {product.title}
        </h3>
        <p className="font-inter text-xs font-normal tracking-tight group-hover:text-gray-700 transition-colors duration-200 ml-2 whitespace-nowrap">
          {formatPrice(product.price)}
        </p>
      </div>
    </Link>

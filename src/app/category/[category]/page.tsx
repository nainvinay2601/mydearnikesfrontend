...
import Link from "next/link";
import ProductFilter from "@/components/major/ProductFilter";
    ...
    return (
      <div className="pt-16">
        ...
        <div className="headingClass py-4 px-[8px] tracking-tight flex flex-col items-center justify-center gap-0 py-8">
          ...
          {/* Category Navigation Menu */}
          <nav className="flex gap-2 mt-6 overflow-x-auto scrollbar-hide px-4 w-full justify-start md:justify-center">
            {categories.map((cat) => {
              const isActive = cat.slug === category;
              return (
                <Link
        ...
        <ProductFilter 
          products={products} 
          categoryTitle={collection?.title || category.replace("-", " ")} 
        />
      </div>
    );
  } catch (error) {
